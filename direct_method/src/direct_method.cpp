...
int main(int argc, char **argv){
    ...
    // generate pixels in rewf and load depth data
    ...
    Sophus::SE3d T_cur_ref; // the transformation from reference image to current image
...
void DirectPoseEstimationSingleLayer(
    ...
    for (int iter = 0; iter < iterations; iter++){
        ...
        // solve update and put into the estimation
    ...
    for (size_t i = 0; i < px_ref.size(); ++i){
        ...
        if (p_cur[0] > 0 && p_cur[1] > 0){
            cv::circle(img2_show, cv::Point2d(p_cur[0], p_cur[1]), 2, cv::Scalar(0, 250, 0), 2);
            cv::line(img2_show, cv::Point2d(p_ref[0], p_ref[1]), cv::Point2d(p_cur[0], p_cur[1]), cv::Scalar(0, 250, 0));
        }
    }
...
void JacobianAccumulator::accumulate_jacobian(const cv::Range &range) {
    ...
    for (size_t i = range.start; i < range.end; i ++) {
        ...
        double X = point_cur[0], Y = point_cur[1], Z = point_cur[2], z2 = Z*Z, Z_inv = 1.0/Z,
                Z2_inv = Z_inv*Z_inv;
        cnt_good++;
